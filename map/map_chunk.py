...
class Chunk:
    def __init__(self, pos):
        self.pos             = pos
        self.tiles           = [[0] * CHUNK_SIZE for _ in range(CHUNK_SIZE)]
        self.render_cache    = None
        self.collision_cache = None

    def _bake_collision(self):
        # Pendiente: combinar tiles adyacentes en colliders compuestos
        self.collision_cache = None

    def _bake_chunk(self, tile_images):
        # Prerenderiza el chunk en una sola Surface para evitar blits individuales cada frame
        self.render_cache = pygame.Surface((CHUNK_SIZE * TILE_SIZE, CHUNK_SIZE * TILE_SIZE))
        for y in range(CHUNK_SIZE):
            for x in range(CHUNK_SIZE):
                tile_id = self.tiles[y][x]
                if tile_id:
                    self.render_cache.blit(tile_images[tile_id], (x * TILE_SIZE, y * TILE_SIZE))
