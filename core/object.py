...
class Object:
    ...
    def _get_scaled_asset(self):
        if self._scaled_asset is None or self._scaled_scale != self.scale:
            if self.scale == 1:
                ...
                self._scaled_asset = pygame.transform.scale(
                    self.asset, (max(1, int(w * self.scale)), max(1, int(h * self.scale))))
    ...
    def _rebuild_cache(self, force=False):
        ...
        # Evita la rotación si no es necesaria — pygame.transform.rotate es costoso
        self._render_asset    = base if self.rotation % 360 == 0 else pygame.transform.rotate(base, self.rotation)
        self._render_rotation = self.rotation
        self._render_scale    = self.scale

    def draw(self, surface, camera):
        self._rebuild_cache()
        rect = self._render_asset.get_rect(center=self.position - camera.position)
