...
class VictoryScene(Scene):
    def __init__(self, kills: int = 0, coins: int = 0):
        super().__init__()
        self._kills = kills
        self._coins = coins
    ...
    def render(self, screen):
        ...
        for text, color in [
            (f"Bajas: {self._kills}",    _INFO_COLOR),
            (f"Monedas: {self._coins}",  (255, 215, 0)),
        ]:
            surf = self._font_sub.render(text, True, color)
            screen.blit(surf, (cx - surf.get_width() // 2, cy))
            cy += 42

        score_surf = self._font_info.render(f"Puntuación final: {self.score}", True, _SCORE_COLOR)
